...
function Band({
  ...
  cardModelPath = '/assets/lanyard/card.glb',
  lanyardWidth = 1,
  anchorOffset = [1.8, 4, 0],
}: BandProps) {
  ...
  const { nodes, materials } = useGLTF(cardModelPath) as any;
  ...
  const cardMap = useMemo(() => {
    ...
    const drawFitted = (img: CanvasImageSource, rect: { x: number; y: number; w: number; h: number }) => {
      ...
      const rw = rect.w * W;
      const rh = rect.h * H;
      const imgWidth = (img as any).width || W;
      const imgHeight = (img as any).height || H;
  ...
  useFrame((state, delta) => {
    ...
    if (fixed.current && j1.current && j2.current && j3.current && card.current) {
      [j1, j2].forEach((ref) => {
        ...
        if (!item.lerped) item.lerped = new THREE.Vector3().copy(item.translation());
        const clampedDistance = Math.max(
          0.1,
          Math.min(1, item.lerped.distanceTo(item.translation()))
        );
        item.lerped.lerp(
          item.translation(),
      ...
      rot.set(currentRotation.x, currentRotation.y, currentRotation.z);
        ...
        >
          ...
          <group
            ...
            onPointerDown={(e) => {
              ...
              const curTrans = card.current.translation();
              setDragged(
                new THREE.Vector3().copy(e.point).sub(vec.set(curTrans.x, curTrans.y, curTrans.z))
              );
            }}
          >
            {nodes?.card && (
        ...
        <Suspense fallback={null}>
          <Physics gravity={gravity} timeStep={isMobile ? 1 / 30 : 1 / 60}>
            <Band
              ...
              cardModelPath={cardModelPath}
              lanyardWidth={lanyardWidth}
              anchorOffset={isMobile ? [0.6, 4, 0] : anchorOffset}
            />
          </Physics>
